...
def propose_assignment_with_minimum_move(config):
    ...
    if config.new_replication_factor > 0:
        logger.info("new_replication_factor= %s",
                    config.new_replication_factor)
    logger.info("topics= %s", config.topics)
    logger.info("partitions=%s", [(t, len(config.topic2partition[t]))
                                  for t in config.topic2partition])
    ...
    for (t, p, b) in config.tpbs:
        if config.pinned_replicas.has_key((t, p, b)):
            proposed_assignment[(t, p, b)] = pulp.LpVariable(
                "{}_{}_B{}".format(t, p, b), 1, 1, 'Integer')
        else:
            proposed_assignment[(t, p, b)] = pulp.LpVariable(
                "{}_{}_B{}".format(t, p, b), 0, 1, 'Integer')
    ...
    for (t, p, b) in config.tpbs:
        if config.current_assignment[(t, p, b)] == 0:
            num_movement += proposed_assignment[(t, p, b)] * \
                config.partition_weights[(t, p)]
        else:
            num_movement += ((proposed_assignment[(t, p, b)] - 1) * -1 *
                             config.partition_weights[(t, p)])
    ...
    for (t, p, b) in config.tpbs:
        _total_replica_weight += config.partition_weights[
            (t, p)] * proposed_assignment[(t, p, b)]
    ...
    for (t, p) in config.tps:
        ...
        if config.new_replication_factor > 0:
            problem += replicas == config.new_replication_factor, "partition (%s,%s) has replication factor %s" % (
                t, p, config.new_replication_factor)
        else:
            problem += replicas == current_replicas, "partition (%s,%s) has replication factor %s" % (
                t, p, current_replicas)

    # c3: new assignment is well_balanced
    for b in config.brokers:
        score = 0
        for (t, p) in config.tps:
            score += config.partition_weights[(t, p)] * \
        ...
        if config.balanced_load_max == config.balanced_load_min:
            ...
            problem += score >= config.balanced_load_min, "load of broker %s is greater than or equal to %s" % (
    ...
    logger.info("Number of Parition Replica Movements: %s",
                pulp.value(problem.objective))

    logger.debug("\n# CURRENT ASSIGNMENT")
    logger.debug(assignment_str(config, config.current_assignment))
    logger.debug("\n# PROPOSED ASSIGNMENT")
    logger.debug(
        assignment_str(
            config,
            dict((k, proposed_assignment[k].value())
                 for k in proposed_assignment)))
...
def assignment_str(config, _assignment):
    ...
    for (t, p) in config.tps:
        str.append("%s_%s\t" % (t, p))
        for b in config.brokers:
            v = config.partition_weights[(t,
                                          p)] if _assignment[(t, p,
                                                              b)] == 1 else 0
            str.append("%s\t" % v)
        str.append("\n")
    return ''.join(str)
...
class ReassignmentOptimizerConfig:
    def __init__(self, _json):
        ...
        self.new_replication_factor = int(
            _json["new_replication_factor"]) if _json.has_key(
                "new_replication_factor") else -1

        _t2p = dict()
        _tp = sorted([(p["topic"], p["partition"])
                      for p in _json["partitions"]["partitions"]],
        ...
        _assignment = dict(
            sum([
                map(lambda r: ((p["topic"], p["partition"], r), 1),
                    p["replicas"]) for p in _json["partitions"]["partitions"]
            ], []))
        ...
        if self.new_replication_factor > 0:
            ...
            self.total_replicas = sum([
                len(p["replicas"]) for p in _json["partitions"]["partitions"]
            ], 0)
            ...
            for j in self.__json["pinned_replicas"]:
                ...
                if not __pinned_replicas.has_key((t, p, r)):
                    if (t, p, r) in self.tpbs:
                        ...
                        __pinned_replicas[(t, p, r)] = 1
                    else:
                        __logger.warning(
                            "broker %s will be vanished in proposed assinment!! replica %s of partition (%s, %s) won't be pinned in proposed assignment!!"
                            % (r, t, p))
        ...
        if self.__json.has_key("balance_parameters"):
            ...
            if self.__json["balance_parameters"].has_key("max_factor"):
                self.balance_max_factor = self.__json["balance_parameters"][
                    "max_factor"]
            else:
                self.balance_max_factor = BALANCE_MAX_FACTOR
        else:
...
if __name__ == '__main__':
    ...
    (problem, status, proposed_assignment
     ) = propose_assignment_with_minimum_move(reassignment_config)
